  ...
  class << self
	...
	def self.create(args)               # This method self.create creates the variable args , which would be a new varialvel
		args.merge!(id: SecureRandom.hex) # cria um string com varios caracteres aleatorios
	  	...
	  	dataset << post              # Now the @@ dataset is receiving this variable args that begins to be counted as new, ie still has nothing in it ,
	...
	def self.load
		@dataset = YAML.load(File.open(dataset_filename, "r")) # o YAMl coloca meu banco de dados em ordem alinhado.
	end
